...
@Component({
  selector: 'app-selecionar-periodos-sintegra',
  templateUrl: './selecionar-periodos-sintegra.component.html',
  styleUrls: ['./selecionar-periodos-sintegra.component.scss']
})
export class SelecionarPeriodosSintegraComponent implements OnInit {

  constructor(private processarService: ProcessarSintegraService
...
//  @Output() AcionarProcessarArquivosGerados = new EventEmitter<any>();
  ...
  GerarEfdIcms() {
    ...
    this.processarService.gerarArqMagneticos(parametrosDados).subscribe((RespostaComMensagemModel) => {
...
GerarSintegra() {
  ...
  this.processarService.gerarArqMagneticosSintegra(parametrosDados).subscribe((RespostaComMensagemModel) => {
    safeCall(RespostaComMensagemModel, () => {
      this.Acionarprocessar.emit();
