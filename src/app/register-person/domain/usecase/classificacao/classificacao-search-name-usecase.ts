...
import { ClassificacaoSearchByName } from '../../entities/classificacao-search.model';
...
import { ShowClassificacaoSearchByNameSate, ClassificacaoErrorState } from '../../state/classificacao-state';
import { ClassificacaoRepository } from '@register/domain/repositories/classificacao-repository';

@Injectable()
export class ClassificacaoSearchNameUseCase implements Usecase<ClassificacaoSearchByName, State> {
    
    constructor(private repository: ClassificacaoRepository) {}

    execute(param: ClassificacaoSearchByName): Observable<State> {
      return Observable.create(obs => {
          const options = this.fillParams(param.pageable)
          const subs = this.repository.findByName(param.query, options).subscribe(
              (reponse) => {
                obs.next(new ShowClassificacaoSearchByNameSate(reponse))
              },
              (error) => {
